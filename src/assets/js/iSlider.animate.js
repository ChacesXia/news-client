export default (iSlider) => {
    ...
    iSlider.extend(iSlider._animateFuncs, {
        ...
        flow: function flow(dom, axis, scale, i, offset) {
            ...
            dom.style.webkitTransform = 'scale(0.85, 0.85) translateZ(' + (offsetRatio * 150 - 150) * Math.abs(i - 1)*.5+ 'px)' + 'translate' + axis + '(' + (offset + scale * (i - 1))*.8 + 'px)' + 'rotate' + rotateDirect + '(' + directAmend * (30 - offsetRatio * 30) * (1 - i) + 'deg)';
        },
